...
if(isset($_POST['nom']))
{
    ...
    if($err == 0)
    {
        ...
        if(isset($_FILES['image']))
        {
            ...
            {
                header("LOCATION:addSkill.php?error=6");
                exit();
            }

            // gestion de l'image
            $dossier = '../images/skills/';
            // fichier.jpg
            ...
            if($err==0)
            {
                ...
                if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier.$fichierCplt))
                {
                    ...
                    /** @var PDO $bdd */
                    $insert = $bdd->prepare("INSERT INTO skills(nom,image) VALUES(:nom,:img)");
                    $insert->execute([
                        ":nom" => $nom,
                        ":img" => $fichierCplt
                    ]);
                    ...
                    header("LOCATION:skills.php?insert=success");
                }else{
                    header("LOCATION:addSkill.php?error=8");
                    exit();
                }
            ...
            header("LOCATION:addSkill.php?error=5");
            exit();
        }
